...
<div class="container-fluid">
	<div class="wrapper">
		<div class="title">TV 게시판</div>
		<div class="searchBar">
			<form action="/tvBoard/hateList" method="get">
		...
		<div class="likeHateText">
			...
			/
			<a href="/tvBoard/hateList" style="color: rgb(91, 189, 250); text-decoration: underline; font-weight: bold;">재미없어요</a>
		</div>
	
		<div class="card-group">
			<c:forEach items="${list }" var="tvbvo">
		    <div class="card-primary text-center card-one">
		      <img class="card-img-top" src="${tvbvo.poster }" alt="${tvbvo.tvTitle }">
		      <div class="card-body">
		        <div class="movieTitle fs-6">${tvbvo.tvTitle }</div>
		        <div class="card-title">${tvbvo.title }</div>
		        <div class="dropdown uDetail">
				  <a href="" data-bs-toggle="dropdown" aria-expanded="false">
				    ${tvbvo.nickName }
				  </a>&nbsp&nbsp<span class="heartColor"><i class="far fa-heart"></i></span>${tvbvo.heart }
				  <ul class="dropdown-menu" aria-labelledby="uDetail">
				    <li><a href="/user/${tvbvo.writer }" class="dropdown-item">정보 보기</a></li>
				  </ul>
				</div>
		        <p class="list-text">조회수 ${tvbvo.readCount }</p>
		        <p class="list-text">${tvbvo.modAt.substring(0, 10) }</p>
		      </div>
		...
		<ul class="pagination justify-content-center">
			<c:if test="${pgn.prev }">
				<li class="page-item">
					<a  class="page-link" href="/tvBoard/hateList?pageNo=${pgn.startPage - 1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">이전</a>
				</li>
			</c:if>
			<c:forEach begin="${pgn.startPage }" end="${pgn.endPage }" var="i">
				<li class="page-item ${pgn.pgvo.pageNo == i ? 'active':''}" aria-current="page">
					<a class="page-link" href="/tvBoard/hateList?pageNo=${i }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">${i }</a>
				</li>
			</c:forEach>
			<c:if test="${pgn.next }">
				<li class="page-item">
					<a class="page-link" href="/tvBoard/hateList?pageNo=${pgn.endPage + 1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">다음</a>
				</li>
			</c:if>
		</ul>
